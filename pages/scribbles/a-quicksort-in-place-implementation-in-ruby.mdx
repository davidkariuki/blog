...
Quicksort is a popular [sorting algorithm](http://en.wikipedia.org/wiki/Sorting_algorithm) with <span className='courier'>O(nlogn)</span> best and average-case
performance, and <span className='courier'>O(n<sup>2</sup>)</span> worst case performance when sorting
<span className='courier'>n</span> elements.
...
successfully partitioned the array around the pivot, <span className='courier'>2</span>.
